...
export default function AdminDashboard() {
  ...
  async function loadDashboard() {
    ...
    if (feedbackResult.error) setError(feedbackResult.error.message);
    else setFeedback(feedbackResult.data || []);

    if (scansResult.error) setError(scansResult.error.message);
    else setScans(scansResult.data || []);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      totalFeedback,
      feedbackRate,
      helpfulPercent: helpfulAnswers.length
        ? Math.round((helpfulYes / helpfulAnswers.length) * 100)
        : 0,
      idPercent: idAnswers.length ? Math.round((idYes / idAnswers.length) * 100) : 0,
    };
  }, [feedback, scans]);
      ...
      <section className="mx-auto max-w-7xl px-6 pb-24 pt-40">
        <div className="mb-12 flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="text-5xl font-black">Analytics Dashboard</h1>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-5">
          <div className="rounded-3xl border border-slate-800 bg-slate-900/50 p-6">
            ...
            <p className="mt-3 text-4xl font-black">{stats.totalScans}</p>
          </div>

          <div className="rounded-3xl border border-slate-800 bg-slate-900/50 p-6">
            <p className="text-sm text-slate-400">Feedback</p>
            <p className="mt-3 text-4xl font-black">{stats.totalFeedback}</p>
          </div>
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-2">
          <div className="rounded-3xl border border-slate-800 bg-slate-900/50 p-6">
            <h2 className="mb-6 text-2xl font-bold">Latest Scans</h2>
            ...
            <div className="space-y-4">
              {scans.slice(0, 10).map((scan) => (
                ...
                >
                  <p className="font-bold">{scan.card_name || "Unknown Card"}</p>
                  <p className="text-sm text-slate-400">
                    Grade: {scan.predicted_grade || "N/A"}
                  </p>
                  <p className="text-sm text-slate-400">
                    PSA 10 Chance: {scan.psa_10_probability || "N/A"}
                  </p>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-4">
              {feedback.slice(0, 10).map((item) => (
                <div
                  ...
                  className="rounded-xl border border-slate-800 bg-slate-950/50 p-4"
                >
                  <p className="font-bold">{item.card_name || "Unknown Card"}</p>
                  <p className="text-sm text-slate-400">
                    AI Grade: {item.predicted_grade || "N/A"}
                  </p>
                  <p className="text-sm text-slate-400">
